...
export class ClientBase extends EventEmitter {
    ...
    private parseTweet(raw: any, depth = 0, maxDepth = 3): Tweet {
        ...
        const t: Tweet = {
            ...
            // FIX: 'language' does not exist in type 'Tweet'
    ...
    async fetchHomeTimeline(
        ...
        // elizaLogger.debug(homeTimeline, { depth: Number.POSITIVE_INFINITY.toString() });
    ...
    async searchWithDelay(
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                console.log(
                    `Fetching search tweets (attempt ${attempt}/${MAX_RETRIES})`
                );
                ...
                if (attempt === MAX_RETRIES) {
                    console.error(
                        "Failed to fetch tweets after all retries:",
                        error
                    );
                    return { tweets: [] };
                }
                console.log(
                    `Attempt ${attempt} failed, retrying in ${DELAY_MS / 1000} seconds...`
                );
