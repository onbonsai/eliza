 ...
 * @param wallet 
 * @param args:
 *      address hook, // address(0) to use default
        bytes calldata token, // abi.encode(string name, string symbol, string uri)
        uint256 initialSupply, // max 10% of initial supply
        CURVE_TYPE curve, // enum: SOFT, NORMAL, STEEP
        address recipient // will be creator of club
 */
export const registerClub = async (
    wallet: Wallet,
    args: {
