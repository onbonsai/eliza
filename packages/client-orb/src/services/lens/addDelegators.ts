...
export const addDelegators = async (
    wallet: Wallet,
    profileId: string,
    delegators
) => {
    ...
    const typedDataResult =
        await client.profile.createChangeProfileManagersTypedData({
    ...
    const signatureTyped = await wallet.createPayloadSignature(
        hashTypedData({
            domain: {
                ...
                verifyingContract: data.typedData.domain
    ...
    // @ts-ignore
    const completion = await client.transaction.waitUntilComplete({
